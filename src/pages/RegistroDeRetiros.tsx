...
import { ArrowLeft, ArrowDownToLine, Minus, Loader2 } from 'lucide-react';
...
export default function RegistroDeRetiros() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const [retirosRes, progressRes] = await Promise.all([
        supabase
          .from('retiros')
      ...
      if (retirosRes.data) {
        setRetiros(retirosRes.data as Retiro[]);
      }
      if (progressRes.data) {
        setSaldoIngresos(Number((progressRes.data as { saldo_ingresos: number }).saldo_ingresos) || 0);
      }
      setLoading(false);
    }
    loadData();
  ...
  useEffect(() => {
    const channel = supabase
      .channel('retiros-and-wallet-changes')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'retiros' },
        (payload) => {
          if (payload.eventType === 'INSERT') {
            const newRow = payload.new as Retiro;
            setRetiros((prev) => {
              if (prev.some((r) => r.id === newRow.id)) return prev;
              return [newRow, ...prev];
            });
          } else if (payload.eventType === 'UPDATE') {
            const updatedRow = payload.new as Retiro;
            setRetiros((prev) =>
            ...
            setRetiros((prev) => prev.filter((r) => r.id !== oldRow.id));
          }
        }
      )
      .on(
        ...
        (payload) => {
          const newProgress = payload.new as { saldo_ingresos: number };
          setSaldoIngresos(Number(newProgress.saldo_ingresos) || 0);
        }
      )
      .subscribe();
  ...
  async function refreshWallet() {
    ...
    const { data } = await supabase
      ...
      .maybeSingle();
    if (data) {
      setSaldoIngresos(Number((data as { saldo_ingresos: number }).saldo_ingresos) || 0);
    }
  }
        ...
        <div className="w-full max-w-lg">
          ...
          >
            ...
            <div className="p-5 flex items-center gap-3" style={{ borderBottom: '1px solid rgba(255,193,7,0.15)' }}>
              ...
              >
                <ArrowDownToLine size={18} style={{ color: '#FFC107' }} />
              </div>
              <h2
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 px-4">
                  <p className="text-sm" style={{ color: '#888888' }}>
                    No tienes retiros registrados todavía.
                  </p>
                </div>
              ) : (
                retiros.map((retiro, index) => {
                  const isExitoso = retiro.status.toLowerCase() === 'exitoso';
                  const shortId = `#${retiro.id.slice(-3).toUpperCase()}`;
                  return (
                    <div
                      key={retiro.id}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        >
                          <Minus size={16} style={{ color: '#FFC107' }} />
                        </div>
                        <div className="min-w-0">
                          ...
                          <p className="text-xs" style={{ color: '#888888' }}>
                            {formatDateTime(retiro.created_at)}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col items-end flex-shrink-0">
                        <p
                          ...
                          style={{ color: '#F472B6' }}
                        >
                          {formatMoney(Number(retiro.amount) || 0)}
                        </p>
                        {isExitoso ? (
                          <span
